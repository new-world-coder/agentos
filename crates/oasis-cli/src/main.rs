...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Commands::Journal { run_id } => {
            ...
            oasis_core::verify_chain(&entries)?;
...
async fn doctor(db: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    // Memory store
    {
        ...
        let run = oasis_core::Run::new("doctor-memory");
    ...
    // SQLite store
    let sqlite_path = db.unwrap_or_else(|| std::env::temp_dir().join("oasis-doctor.db"));
    {
        let store = SqliteStore::open(&sqlite_path)?;
        let run = oasis_core::Run::new("doctor-sqlite");
        store.create_run(&run).await?;
        let tip = store.tip_hash(&run.id).await?;
        assert_eq!(tip, oasis_core::Hash::genesis());
