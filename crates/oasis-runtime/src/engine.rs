...
use oasis_policy::{PolicyDecision, PolicyEngine};
...
impl Default for RuntimeConfig {
    fn default() -> Self {
        Self {
            max_steps: 32,
            system_prompt: "You are Oasis, a durable agent. Propose effects carefully.".into(),
        }
    }
}
...
impl Runtime {
    ...
    pub async fn resume(&self, run_id: &RunId) -> Result<StepOutcome> {
        ...
        oasis_core::verify_chain(&journal)?;
    ...
    async fn handle_proposed(
        &self,
        run: &mut Run,
        proposed: oasis_core::ProposedEffect,
...
mod tests {
    use super::*;
    use oasis_provider::{MockProvider, MockScript};
    use oasis_store::{MemoryStore, SqliteStore};
    ...
    async fn completes_with_mock_provider() {
        ...
        oasis_core::verify_chain(&journal).unwrap();
