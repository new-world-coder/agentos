...
use oasis_core::{Effect, Hash, JournalEntry, Result, Run, RunId};
...
impl Store for MemoryStore {
    ...
    async fn update_run(&self, run: &Run) -> Result<()> {
        ...
        if !g.runs.contains_key(&run.id.0) {
            return Err(oasis_core::CoreError::RunNotFound(run.id.0.clone()));
        }
        g.runs.insert(run.id.0.clone(), run.clone());
        Ok(())
    }
    ...
    async fn append_journal(&self, entry: &JournalEntry) -> Result<()> {
        ...
        if entry.prev_hash != expected_prev {
            return Err(oasis_core::CoreError::InvalidJournalChain(format!(
